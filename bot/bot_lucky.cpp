...
// ====================== SCORING CONSTANTS (same as fever) ======================
...
constexpr int MC_DEPTH                     = 4;     // deterministic MC pieces (pieces 3-6)
...
QuickResult bfs_piece_eval(const PlayerBoard& board) {
    QuickResult best;
    ...
    while (!queue.empty()) {
        ...
        if (!dup) {
            ...
            if (vc > best.vc) is_better = true;
            else if (vc == best.vc && chains > best.chains) is_better = true;
            else if (vc == best.vc && chains == best.chains && score > best.score) is_better = true;
...
// Use the board's actual RNG state to predict exact future piece colors.
// This gives us perfect-information simulation instead of random sampling.

int64_t deterministic_mc_eval(const PlayerBoard& board_after_p2) {
    PlayerBoard sim = board_after_p2;
    int64_t total_score = 0;

    for (int piece = 0; piece < MC_DEPTH; piece++) {
        // Generate the ACTUAL next piece colors from the game's RNG
        sim.cap.h1 = sim.board_rnd_color();
        sim.cap.h2 = sim.board_rnd_color();
        sim.cap.r = 0;
        sim.cap.c = COLS / 2 - 1;
        sim.cap.orient = 0;

        if (!sim.fits(sim.cap)) {
            total_score -= 200'000'000LL;
            break;
        }

        // Full BFS to find best placement
        QuickResult result = bfs_piece_eval(sim);
        total_score += result.score;
        sim = result.board_after;

        if (sim.cleared_viruses >= sim.total_viruses) {
...
Move LuckyBot::get_move(const PlayerBoard& board) {
    ...
    std::vector<P1Result> piece1_results;
    ...
    for (int i = 0; i < k; i++) {
        ...
        QuickResult second = bfs_piece_eval(board2);
    ...
    std::sort(piece1_results.begin(), piece1_results.end(),
              [](const P1Result& a, const P1Result& b) {
                  return a.combined_score > b.combined_score;
              });

    // Step 3: Deterministic Monte Carlo evaluation for top candidates
    // Uses the board's actual RNG state to predict exact future piece colors
    int eval_count = std::min((int)piece1_results.size(), DEEP_EVAL_TOP);

    int best_idx = piece1_results[0].idx;
    int64_t best_total_score = std::numeric_limits<int64_t>::min();

    for (int i = 0; i < eval_count; i++) {
        P1Result& p1r = piece1_results[i];
        Candidate& first = candidates[p1r.idx];

        if (first.board_after.cleared_viruses >= first.board_after.total_viruses) {
            if (p1r.combined_score > best_total_score) {
                best_total_score = p1r.combined_score;
                best_idx = p1r.idx;
            }
            continue;
        }
        ...
        // Deterministic MC: simulate pieces 3-6 using the ACTUAL RNG state
...
static bool lucky_bot_registered = [] {
    BotRegistry::instance().register_bot(
        "lucky",
        "Deterministic Monte Carlo with RNG-predicted future pieces",
        []() -> std::unique_ptr<Bot> { return std::make_unique<LuckyBot>(); }
    );
    return true;
}();
